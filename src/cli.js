...
let web3, ajax, circuit, proving_key, groth16, erc20, senderAccount, netId
...
async function deposit({ currency, amount }) {
  ...
  const noteString = `ajax-${currency}-${amount}-${netId}-${note}`
  console.log(`Your note: ${noteString}`)
  if (currency === 'eth') {
    await printETHBalance({ address: ajax._address, name: 'Ajax' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
    const value = isLocalRPC ? ETH_AMOUNT : fromDecimals({ amount, decimals: 18 })
    console.log('Submitting deposit transaction')
    await ajax.methods.deposit(toHex(deposit.commitment)).send({ value, from: senderAccount, gas: 2e6 })
    await printETHBalance({ address: ajax._address, name: 'Ajax' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
  } else { // a token
    await printERC20Balance({ address: ajax._address, name: 'Ajax' })
    ...
    const allowance = await erc20.methods.allowance(senderAccount, ajax._address).call({ from: senderAccount })
    console.log('Current allowance is', fromWei(allowance))
    if (toBN(allowance).lt(toBN(tokenAmount))) {
      console.log('Approving tokens for deposit')
      await erc20.methods.approve(ajax._address, tokenAmount).send({ from: senderAccount, gas: 1e6 })
    }

    console.log('Submitting deposit transaction')
    await ajax.methods.deposit(toHex(deposit.commitment)).send({ from: senderAccount, gas: 2e6 })
    await printERC20Balance({ address: ajax._address, name: 'Ajax' })
    await printERC20Balance({ address: senderAccount, name: 'Sender account' })
  }
 ...
 * Download deposit events from the ajax, reconstructs merkle tree, finds our deposit leaf
 * in it and generates merkle proof
 * @param deposit Deposit object
 */
async function generateMerkleProof(deposit) {
  // Get all deposit events from smart contract and assemble merkle tree from them
  console.log('Getting current state from ajax contract')
  const events = await ajax.getPastEvents('Deposit', { fromBlock: 0, toBlock: 'latest' })
  const leaves = events
  ...
  const isValidRoot = await ajax.methods.isKnownRoot(toHex(root)).call()
  const isSpent = await ajax.methods.isSpent(toHex(deposit.nullifierHash)).call()
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      const relay = await axios.post(relayerURL + '/relay', { contract: ajax._address, proof, args })
      if (netId === 1 || netId === 42) {
    ...
    await ajax.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
...
function parseNote(noteString) {
  const noteRegex = /ajax-(?<currency>\w+)-(?<amount>[\d.]+)-(?<netId>\d+)-0x(?<note>[0-9a-fA-F]{124})/g
...
async function loadDepositData({ deposit }) {
  try {
    const eventWhenHappened = await ajax.getPastEvents('Deposit', {
      filter: {
    ...
    const isSpent = await ajax.methods.isSpent(deposit.nullifierHex).call()
...
async function loadWithdrawalData({ amount, currency, deposit }) {
  try {
    const events = await ajax.getPastEvents('Withdrawal', {
      fromBlock: 0,
      toBlock: 'latest',
    })
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  let contractJson, erc20ContractJson, erc20ajaxJson, ajaxAddress, tokenAddress
  // TODO do we need this? should it work in browser really?
  if (inBrowser) {
    // Initialize using injected web3 (Metamask)
    // To assemble web version run `npm run browserify`
    web3 = new Web3(window.web3.currentProvider, null, { transactionConfirmationBlocks: 1 })
    contractJson = await (await fetch('build/contracts/ETHAjax.json')).json()
    ...
    contractJson = require(__dirname + '/../build/contracts/ETHAjax.json')
    ...
    erc20ajaxJson = require(__dirname + '/../build/contracts/ERC20Ajax.json')
  }
  ...
  if (isLocalRPC) {
    ajaxAddress = currency === 'eth' ? contractJson.networks[netId].address : erc20ajaxJson.networks[netId].address
    tokenAddress = currency !== 'eth' ? erc20ContractJson.networks[netId].address : null
    senderAccount = (await web3.eth.getAccounts())[0]
  } else {
    try {
      ajaxAddress = config.deployments[`netId${netId}`][currency].instanceAddress[amount]
      if (!ajaxAddress) {
        throw new Error()
      }
      tokenAddress = config.deployments[`netId${netId}`][currency].tokenAddress
    } catch (e) {
      console.error('There is no such ajax instance, check the currency and amount you provide')
      process.exit(1)
    }
  }
  ajax = new web3.eth.Contract(contractJson.abi, ajaxAddress)
...
async function main() {
  if (inBrowser) {
    ...
    program
      ...
      .description('Submit a deposit of specified currency and amount from default eth account and return the resulting note. The currency is one of (ETH|DAI|cDAI|USDC|cUSDC|USDT). The amount depends on currency, see config.js file.')
