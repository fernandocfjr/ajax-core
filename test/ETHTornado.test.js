...
const Ajax = artifacts.require('./ETHAjax.sol')
...
contract('ETHAjax', (accounts) => {
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const etherDenomination = await ajax.denomination()
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await ajax.deposit(commitment, { value, from: sender })
      ...
      ;({ logs } = await ajax.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await ajax.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await ajax.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await ajax.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await ajax.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      ...
      const balanceAjaxBefore = await web3.eth.getBalance(ajax.address)
      ...
      let isSpent = await ajax.isSpent(toFixedHex(input.nullifierHash))
      isSpent.should.be.equal(false)

      // Uncomment to measure gas usage
      // gas = await ajax.withdraw.estimateGas(proof, publicSignals, { from: relayer, gasPrice: '0' })
      ...
      const { logs } = await ajax.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceAjaxAfter = await web3.eth.getBalance(ajax.address)
      ...
      balanceAjaxAfter.should.be.eq.BN(toBN(balanceAjaxBefore).sub(toBN(value)))
      ...
      isSpent = await ajax.isSpent(toFixedHex(input.nullifierHash))
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      tree.insert(deposit.commitment)
      await ajax.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await ajax.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await ajax.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await ajax.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await ajax.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await ajax.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await ajax.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await ajax.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await ajax.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await ajax.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
      ...
      const spentArray = await ajax.isSpentArray([nullifierHash1, nullifierHash2])
