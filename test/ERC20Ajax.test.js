...
const Ajax = artifacts.require('./ERC20Ajax.sol')
...
contract('ERC20Ajax', (accounts) => {
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const tokenFromContract = await ajax.token()
  ...
  describe('#deposit', () => {
    it('should work', async () => {
      ...
      await token.approve(ajax.address, tokenDenomination)

      let { logs } = await ajax.deposit(commitment, { from: sender })
    ...
    it('should not allow to send ether on deposit', async () => {
      ...
      let error = await ajax.deposit(commitment, { from: sender, value: 1e6 }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      await token.approve(ajax.address, tokenDenomination, { from: user })
      // Uncomment to measure gas usage
      // let gas = await ajax.deposit.estimateGas(toBN(deposit.commitment.toString()), { from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await ajax.deposit(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })
      ...
      const balanceAjaxBefore = await token.balanceOf(ajax.address)
      ...
      const { logs } = await ajax.withdraw(proof, ...args, { value: refund, from: relayer, gasPrice: '0' })

      const balanceAjaxAfter = await token.balanceOf(ajax.address)
      ...
      balanceAjaxAfter.should.be.eq.BN(toBN(balanceAjaxBefore).sub(toBN(tokenDenomination)))
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let { reason } = await ajax.withdraw(proof, ...args, { value: 1, from: relayer, gasPrice: '0' })
        .should.be.rejected
      reason.should.be.equal('Incorrect refund amount received by the contract')
      ;({ reason } = await ajax.withdraw(proof, ...args, {
    ...
    it.skip('should work with REAL USDT', async () => {
      ...
      await usdtToken.approve(ajax.address, tokenDenomination, { from: user })
      console.log('approve done')
      const allowanceUser = await usdtToken.allowance(user, ajax.address)
      ...
      const balanceAjaxBefore = await usdtToken.balanceOf(ajax.address)
      ...
      const ethBalanceReceiverBefore = await web3.eth.getBalance(toFixedHex(recipient, 20))
      let isSpent = await ajax.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(false)
      ...
      const balanceAjaxAfter = await usdtToken.balanceOf(ajax.address)
      ...
      logs[0].args.fee.should.be.eq.BN(feeBN)
      isSpent = await ajax.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(true)
    })
